...
## Date : 2023-01-23 18:35
## Description: A Contact App connected to a Postgres Database
...
# Connect to the relevant instance of the Postgres Server

conn = pg2.connect("dbname=project_contacts  user=postgres  port=5433") 
cur = conn.cursor()
...
sql_cont = '''
        ...
        CREATE TABLE IF NOT EXISTS contact_book (
            s_no serial primary key not null  ,
            name text,
            email text,
            phone_number text ,
            address text
...
def submit_record():
    ...
    if name=='' or email=='' or phone=='' or address=='':
        ...
        cur.execute(
        "INSERT INTO contact_book (name, email, phone_number, address) VALUES (%s,%s,%s,%s)", (name, email, phone, address))
        conn.commit()
...
def list_contacts():
    cur.execute('SELECT name FROM contact_book')
    fetch = cur.fetchall()
...
def delete_record():
    global listbox, conn, cur

    if not listbox.get(ACTIVE):
        mb.showerror("No item selected", "You have not selected any item!")

    cur.execute('DELETE FROM CONTACT_BOOK WHERE NAME=?', (listbox.get(ACTIVE)))
    conn.commit()
...
def delete_all_records():
    cur.execute('DELETE FROM CONTACT_BOOK')
    conn.commit()
...
def view_record():
    ...
    curr = cur.execute('SELECT * FROM CONTACT_BOOK WHERE NAME=%s', listbox.get(ACTIVE))
...
def search():
   ...
   if query != '':
    ...
    curr = conn.execute('SELECT * FROM CONTACT_BOOK WHERE NAME LIKE %s', ('%'+query+'%', ))
    check = curr.fetchall()
...
## Set up and initialze the GUI Window for the database

root = Tk()
root.title('RADFS Contact Book')
...
root.mainloop()
conn.close()  ##close connection to Postgres
